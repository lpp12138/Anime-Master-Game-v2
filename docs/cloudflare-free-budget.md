...
首页密钥上传一套 N 张题库固定产生 N 次 Worker 图片请求、N 次 R2 `PutObject`，以及 1 次 finalize Worker 请求和 N 次 R2 `head()`；不进入 Room DO、不产生 Alarm 或广播，也不随房间 50 人放大。每张题同时写入 1 行有界 `question_image_index`，番剧和最多 8 个角色都保存在该行，禁止按角色展开为 N×8 行。满 30 张时产生 1 行 manifest 和 30 行图片索引数据；若把表数据、主键、稳定投稿 ID 唯一索引、番剧 partial index 及公开题库现有索引均按各 1 行保守计算，约为 130 rowsWritten/套。每天 60 套约 7,800 行（7.8%），实际仍以 D1 Analytics 为准；这种单行 JSON 方案用于避免逐标签规范化表在极端情况下超过每日写额度。
...
不含标签辅助时，N 上限 30 的单套最多 31 次 Worker 上传/finalize 请求、30 次 Class A 写入和 30 次 Class B 读取。按每天 60 套满 30 张估算，每天 1,860 次 Worker 上传/finalize 请求，每月 54,000 次 R2 Class A/54,000 次 Class B 操作；Class A 占每月 100 万免费额度 5.4%。极端文件上限为每套 300MB，但正常浏览器会先将每张图限制到约 207 万像素并转为 WebP/JPEG。finalize JSON 最大 512 KiB；manifest 与全部逐图索引在一个 D1 batch 中原子写入，任何语句失败时整批为 0 行，不再执行补偿删除。浏览器在同内容的一次投稿重试期间复用已上传的 R2 key 和稳定投稿 ID，Worker 以内容指纹拒绝同 ID 的变更；若成功响应丢失，普通串行重试只增加 1 次 Worker 请求和 1 次唯一索引读取，不再产生 R2 put/head、Bangumi 上游访问或 D1 写入。真正同时到达的相同投稿请求仍可能各自执行校验和 R2 head，但唯一约束保证只有一批写入成功，其他请求读取并返回同一题库。finalize 失败或被放弃的对象不被题库引用，继续由 72 小时 R2 对账清理。该入口只允许服务端 secret 鉴权，密钥泄露时必须立即轮换；密钥和普通 R2 上传均不得通过 `ListObjects` 做同步配额扫描。
